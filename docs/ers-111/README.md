# ERS-111 Research

This directory is the working documentation set for ERS-111 media,
forensics, and boot experiments.

The working assumption is that ideas from
`cartheur-aibo/openr-debian` are useful as a method, but not as a
drop-in deployment recipe for a first-generation Aibo.

## Scope

- `media.md` tracks removable-media assumptions and what must be confirmed on the real unit
- `forensics.md` tracks preservation and boot-layout inventory work
- `experiments.md` tracks staged boot and payload testing, including R-Code as an early ERS-11x option
- `r-code.md` tracks the local Sony R-Code 110/111 package and why it matters

## Immediate Goal

The next milestone is simple: preserve the original media and describe
its boot layout well enough to decide whether an ERS-111-specific
equivalent to the `openr-debian` workflow is realistic.
